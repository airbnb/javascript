...
export const a11y: Linter.RulesRecord = {
  /**
   * Enforce all elements that require alternative text have
   * meaningful information to relay back to end user.
  ...
  /**
   * Enforce that elements that do not support ARIA roles,
  ...
  /**
   * Enforce that `onMouseOver`/`onMouseOut` are accompanied
   * by `onFocus`/`onBlur` for keyboard-only users.
