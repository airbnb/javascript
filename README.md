...
## References
  ...
  - [2.1](#references--prefer-const) We do recommend using `const`, but eslint will simply warn you about a variable that is never reassigned; avoid using `var`. eslint: [`prefer-const`](http://eslint.org/docs/rules/prefer-const.html)

    > Why? `const` should be used with values that are not meant to be changed. 
...
## Functions
  ...
  - [7.1](#functions--declarations) Use function declarations instead of function expressions. eslint: [`func-style`](http://eslint.org/docs/rules/func-style)
...
## Arrow Functions
  ...
  - [8.2](#arrows--implicit-return) Allways use parens and and curly braces in arrow functions. eslint: [`arrow-parens`](http://eslint.org/docs/rules/arrow-parens.html), [`arrow-body-style`](http://eslint.org/docs/rules/arrow-body-style.html) 

    > Why? While there are cases where a single line is an implicit return and you don't need the parens for a single argument, we want to keep our code clean and consisitant. This will also easily allow for extending functionality with out have to refactor the function signature.
  ...
  - [8.3](#arrows--paren-wrap) ~~In case the expression spans over multiple lines, wrap it in parentheses for better readability~~. Since we do not allow for arrow functions without curly braces, this is not applicable. 


  <a name="arrows--one-arg-parens"></a><a name="8.4"></a>
  - [8.4](#arrows--one-arg-parens) Always include parens around arguments eslint: [`arrow-parens`](http://eslint.org/docs/rules/arrow-parens.html) 
  ...
  - [8.5](#arrows--confusing) ~~Avoid confusing arrow function syntax (`=>`) with comparison operators (`<=`, `>=`).~~ Because we require curly braces, this is not a factor. But it is good be be aware of when dealling with other sources. eslint: [`no-confusing-arrow`](http://eslint.org/docs/rules/no-confusing-arrow)
...
## Classes & Constructors
  ...
  - [9.1](#constructors--use-class) ~~Always use `class`. Avoid manipulating `prototype` directly.~~ We prefer a functional approach and suggest avoiding ES6 classes. [How to Use Classes and Sleep at Night](https://medium.com/@dan_abramov/how-to-use-classes-and-sleep-at-night-9af8de78ccb4#.z87vv2s1w)


  <a name="constructors--extends"></a><a name="9.2"></a>
  - [9.2](#constructors--extends) If you find you need to work with classes and you find that you need to extend it's functionality, then `extends` should be used. Never extend and class that is already extended. [See](https://medium.com/@dan_abramov/how-to-use-classes-and-sleep-at-night-9af8de78ccb4#.z87vv2s1w) for more information.
  ...
  - [9.4](#constructors--tostring) ~~It's okay to write a custom toString() method, just make sure it works successfully and causes no side effects.~~ Nope. We do not allow for this pattern.
...
## Iterators and Generators
  ...
  - [11.3](#generators--spacing) If you must use generators, (#generators--nope), make sure their function signature is spaced properly. eslint: [`generator-star-spacing`](http://eslint.org/docs/rules/generator-star-spacing)
...
## Variables
  ...
  - [13.2](#variables--one-const) Use multiple variable declartions. eslint: [`one-var`](http://eslint.org/docs/rules/one-var.html) jscs: [`disallowMultipleVarDecl`](http://jscs.info/rule/disallowMultipleVarDecl)
  ...
  - [13.4](#variables--define-where-used) Assign at the top of their scope. For `var`, top of `function`. For `let`s and `const`s, define at the top of their block scope;
...
## Blocks
  ...
  - [16.1](#blocks--braces) Always use braces.
  
  > Why? To avoid issues later on, when functionality needs to be extended.
...
## Comments
  ...
  - [17.1](#comments--multiline) Use `/* ... */` for multi-line comments (not JSDOC blocks)
    ...
    // good CHANGE this to refelct satndard block comment
    /**
     * make() returns a new element
     * based on the passed in tag name
     *
  ...
  <a name="comments--singleline"></a><a name="17.3"></a>
  - [17.2](#comments--dockblock) We use [JSDOCs](http://usejsdoc.org/) for documentation. All function and methods should be documented whether or not they are private.
...
## Whitespace
  ...
  - [18.1](#whitespace--spaces) We use Hard Tabs. eslint: [`indent`](http://eslint.org/docs/rules/indent.html) 
  ...
  - [18.6](#whitespace--chains) (single indent) Use indentation when making long method chains (more than 2 method chains). Use a leading dot, which
  ...
  - [18.8](#whitespace--padded-blocks) Pad your blocks with blank lines. eslint: [`padded-blocks`](http://eslint.org/docs/rules/padded-blocks.html) 
  ...
  <a name="whitespace--in-braces"></a><a name="18.12"></a>
  - [18.11](#whitespace--newline-props) When an object has multiple properties, they sould be placed on separate lines. eslint: [`object-property-newline`](http://eslint.org/docs/rules/object-property-newline) 

> Why? This makes for better readability, especially in diffs.
  ...
  <a name="whitespace--max-len"></a><a name="18.13"></a>
...
<a name="commas--leading-trailing"></a><a name="19.1"></a>
  ...
  - [19.2](#commas--dangling) **No** Additional trailing comma: eslint: [`comma-dangle`](http://eslint.org/docs/rules/comma-dangle.html) 
...
## Naming Conventions
  ...
  <a name="naming--PascalCase"></a><a name="22.3"></a>
  - [22.3](#naming--UpperSnakeCase) Use UPPER_SNAKE_CASE only when naming `const`s.  

    ```javascript
    // bad
    const VampireSlayer = 'Buffy Summers';
    
    // bad
    const vampireSlayer = 'Kendra';
    
    // bad
    const vampire_slayer = 'Faith Lehane';
      
    // good
    const VAMPIRE_SLAYER = 'Buffy Summers';
...
TODO: This will be updated when we have completed our testing framework setup
